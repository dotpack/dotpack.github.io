...
function initializeGame() {
  ...
  document.getElementById(
    "generation"
  ).innerText = `Поколение: ${generationCount}`;
}
...
function fastGame() {
  if (intervalId) {
    ...
    intervalId = setInterval(generateNextGeneration, 20); // Иначе запускаем с интервалом 200 миллисекунд
  }
}
...
function createGrid() {
  ...
  cells = Array.from({ length: gridSize * gridSize }, () => false);
...
function toggleCell(event) {
  ...
  const x = Math.floor(
    (event.clientX - rect.left - 10) / ((canvas.width - 20) / gridSize)
  );
  const y = Math.floor(
    (event.clientY - rect.top - 10) / ((canvas.height - 20) / gridSize)
  );
...
function resizeCanvas() {
  ...
  const w = window.innerWidth * 0.95; // Устанавливаем ширину canvas как 95% ширины окна
  const h = window.innerHeight * 0.75; // Устанавливаем высоту canvas как 75% высоты окна
